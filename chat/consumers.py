...
class ChatConsumer(WebsocketConsumer):
    ...
    def broadcast(self, event: Event):
        ...
        async_to_sync(self.channel_layer.group_send)('default', {"type": "SendMessage", "message": event})
    ...
    def websocket_connect(self, message):
        ...
        async_to_sync(self.channel_layer.group_add)('default', self.channel_name)

    def websocket_receive(self, message):
        data = json.loads(message['text'])

        if data['type'] == 'init':
            self.broadcast(Event.notice(f"@{data['data']['sender']['username']} 进入聊天室"))
    ...
    def websocket_disconnect(self, message):
        async_to_sync(self.channel_layer.group_discard)('default', self.channel_name)
        raise StopConsumer()
